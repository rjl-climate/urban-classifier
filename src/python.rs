...
impl PyUrbanClassifier {
    ...
    fn new(wudapt_path: &str) -> PyResult<Self> {
        let inner = UrbanClassifier::new(wudapt_path)
            .map_err(|e| convert_classifier_error_to_py(e))?;
    ...
    fn run_classification(
        ...
        let result_df = self.inner.run_classification(
            &df.0,
            station_id_col,
            lon_col,
            lat_col,
    ...
    fn validate_dataframe(
        ...
        let _result = self.inner.run_classification(
            &df.0,
            station_id_col,
            lon_col,
            lat_col,
...
fn convert_classifier_error_to_py(error: ClassifierError) -> PyErr {
    match error {
        ...
        ClassifierError::InvalidCoordinate { lon, lat } => {
            pyo3::exceptions::PyValueError::new_err(format!(
                "Invalid coordinate: longitude={}, latitude={}",
                lon, lat
            ))
        }
        ClassifierError::SchemaValidation { message } => {
            pyo3::exceptions::PyValueError::new_err(format!("Schema validation failed: {}", message))
        }
        ...
        ClassifierError::CoordinateTransform { message } => {
            pyo3::exceptions::PyRuntimeError::new_err(format!("Coordinate transformation failed: {}", message))
        }
        ClassifierError::RasterSampling { pixel, line, message } => {
            pyo3::exceptions::PyRuntimeError::new_err(format!(
                "Raster sampling failed at ({}, {}): {}",
                pixel, line, message
            ))
        }
        ClassifierError::OverrideApplication { station_id, message } => {
            pyo3::exceptions::PyValueError::new_err(format!(
...
pub fn urban_classifier_module(_py: Python<'_>, m: &PyModule) -> PyResult<()> {
    ...
    m.add("__description__", "High-performance urban classification using Local Climate Zone (LCZ) system")?;
