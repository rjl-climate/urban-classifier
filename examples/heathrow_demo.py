...
def classify_heathrow():
    ...
    print(f"📍 Location: Heathrow Airport, London, UK")
    ...
    for path in wudapt_paths:
        if Path(path).exists():
            ...
            if file_size > 3_000_000_000:  # At least 3GB (should be ~4GB)
                ...
                print(f"⚠️  Found {path} but it's too small ({file_size/1e9:.1f}GB) - likely incomplete")
    ...
    if wudapt_file:
        ...
        try:
            ...
            if lcz_code == 8:
                ...
                print("   • Large buildings (terminals) with extensive open areas (runways)")
                ...
                print("   • Unexpected for Heathrow - may indicate data resolution limits")
